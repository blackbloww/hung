...
<main class="flex">
    ...
    <div class="max-w-[100rem] w-full mx-auto py-[5rem]">
        ...
        <div class="bg-gradient-to-r from-blue-600 to-indigo-800 text-white rounded-t-2xl shadow-lg p-6">
            <div class="font-bold text-center text-[2rem]">CHỈNH SỬA THÔNG TIN SẢN PHẨM</div>
        </div>
                ...
                <div id="comment-form">
                    ...
                    <?php else: ?>
                        <?php foreach ($comments as $comment): ?>
                            <div class="bg-gray-100 p-8 rounded-xl space-y-8 mt-8 comment-item">
                                <div class="form-group">
                                    ...
                                    <input type="file" id="comment_avata[]" name="comment_avata[]"  <?= empty($comment['avatar']) ? 'required' : '' ?>
                                    ...
                                    <img 
                                        class="preview mt-3 w-[10rem] h-[10rem] object-cover rounded-lg <?= $comment['avatar'] ? '' : 'hidden' ?>" 
                                        src="<?= $comment['avatar'] ? '/admin/uploads/comments/' . $comment['avatar'] : '' ?>" 
                                    />
                                    <input type="hidden" name="old_comment_avatar[]" value="<?= $comment['avatar'] ?>">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="file" id="comment_product_image[]" name="comment_product_image[]" <?= empty($comment['product_image']) ? 'required' : '' ?>
                                        class="bg-white max-w-[29.5rem] w-full px-4 py-3 text-[1.4rem] border border-gray-300 rounded-lg focus:ring-blue-300 focus:border-blue-500 transition-all duration-300">
                                    <img 
                                        class="preview mt-3 w-[10rem] h-[10rem] object-cover rounded-lg <?= $comment['product_image'] ? '' : 'hidden' ?>" 
                                        src="<?= $comment['product_image'] ? '/admin/uploads/comments/' . $comment['product_image'] : '' ?>" 
                                    />
                                    <input type="hidden" name="old_comment_product_image[]" value="<?= $comment['product_image'] ?>">
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                ...
                <div id="content-form">
                    ...
                    <?php else: ?>
                        <?php foreach ($contents as $content): ?>
                            <div class="bg-gray-100 p-8 rounded-xl space-y-8 mt-8 content-item">
                                <div class="form-group">
                                    <button type="button"
                                    ...
                                    <input type="file" id="content_image" name="content_image[]"  <?= empty($content['images']) ? 'required' : '' ?>
                                        class="bg-white max-w-[29.5rem] w-full px-4 py-3 text-[1.4rem] border border-gray-300 rounded-lg focus:ring-blue-300 focus:border-blue-500 transition-all duration-300">
                                    <img 
                                        class="preview mt-3 w-[10rem] h-[10rem] object-cover rounded-lg <?= $content['images'] ? '' : 'hidden' ?>" 
                                        src="<?= $content['images'] ? '/admin/uploads/content/' . $content['images'] : '' ?>" 
                                    />
                                    <input type="hidden" name="old_content_images[]" value="<?= $content['images'] ?>">
                                </div>
                ...
                <div id="slide-form">
                    ...
                    <?php else: ?>
                        <?php foreach ($slides as $slide): ?>
                            <div class="bg-gray-100 p-8 rounded-xl space-y-8 mt-8 slide-item">
                                <div class="form-group">
                                    <button type="button"
                                    ...
                                    <input type="file" id="slide_image" name="slide_image[]" <?= empty($slide['images']) ? 'required' : '' ?>
                                        class="bg-white max-w-[29.5rem] w-full px-4 py-3 text-[1.4rem] border border-gray-300 rounded-lg focus:ring-blue-300 focus:border-blue-500 transition-all duration-300"
                                        placeholder="Nhập tên sản phẩm">
                                    <img 
                                        class="preview mt-3 w-[10rem] h-[10rem] object-cover rounded-lg <?= $slide['images'] ? '' : 'hidden' ?>" 
                                        src="<?= $slide['images'] ? '/admin/uploads/slide/' . $slide['images'] : '' ?>" 
                                    />
                                    <input type="hidden" name="old_slide_image[]" value="<?= $slide['images'] ?>">
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                ...
                function setupCloneSystem(containerId, itemClass, cloneBtnId, removeClass) {

                    const cloneBtn = document.getElementById(cloneBtnId);
                    const container = document.getElementById(containerId);

                    if (!cloneBtn || !container) return;

                    const originalForm = container.querySelector("." + itemClass);
                    ...
                    container.addEventListener("click", function (e) {
                        if (e.target.classList.contains(removeClass)) {
                            ...
                            const forms = container.querySelectorAll("." + itemClass);

                            // // ❗ Không cho xóa nếu chỉ còn 1
                            // if (forms.length <= 1) {
                            //     alert("Không thể xóa vì phải có ít nhất 1 mục!");
                            //     return;
                            // }

                            const form = e.target.closest("." + itemClass);
                            if (form) form.remove();
                        }
                    });
                    ...
                    cloneBtn.addEventListener("click", function () {
                        ...
                        clone.querySelectorAll(".preview").forEach(img => {
                            img.src = "";
                ...
                setupCloneSystem("comment-form", "comment-item", "cloneFormBtn", "remove-form-btn");
                setupCloneSystem("content-form", "content-item", "cloneContentForm", "remove-content-form");
                setupCloneSystem("slide-form", "slide-item", "cloneSlideForm", "remove-slide-form");
